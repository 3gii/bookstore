        ...
        if (data){
            ...
            if(len>0) {
                for (var i = 0; i < len; i++) {
                    item = data[i];
                    str += '<tr>' +
                        '<td data-id="' + item.id + '"><a href="order_detail.html?id='+item.id+'">' + item.order_no + '</a></td>' +
                        '<td class="center"><span class="block">' + item.create_time + '</span></td>' +
                        '<td class="center"><span class="block">'+ item.snap_name +'</td>'+
                        '<td class="center"><span><i>￥</i><b>'+ item.total_price +'</b></span></td>'+
                        '<td>' + getOrderStatus(item.status) + '</td>' +
                        '<td data-id="' + item.id + '" >' + getBtns(item.status) +'</td>'+
                        '<td><div data-id="' + item.id + '" class="BatchOperation fl del"><input type="button" id="del" value="删除" class="btnStyle"/></div></td>'
                        '</tr>';
                }
            }
            else{
