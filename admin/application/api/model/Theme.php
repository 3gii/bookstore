...
{
    protected $hidden = ['delete_time', 'head_img_id'];
...
//            ->with('products.imgs')
    ...
    public static function updateTheme($id,$name,$description,$topic_img_id)
    {
       
        $theme = self::get($id);
        
        $result=ImageService::deleteImage($theme->topic_img_id);
        if (!$theme && $result>0)
        {
